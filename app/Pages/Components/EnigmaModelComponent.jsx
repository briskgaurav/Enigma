...
export default function EnigmaModelComponent({}) {
  ...
  const brustOFF = () => {
    if (isBrust) {
      ...
      [
        ...
        { ref: meshPart3Ref, props: { x: 0, y: 0, duration: 1 } },
        { ref: meshPart4Ref, props: { x: 0, y: 0, duration: 1 } },
      ].forEach(({ ref, props }) => {
        gsap.to(ref.current.position, {
          ...props,
          ease: "back.out(2)",
  ...
  const materialsProps = useControls({
    thickness: { value: 1.35, min: 0, max: 3, step: 0.05 },
